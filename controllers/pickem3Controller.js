...
exports.getAllPicks = factoryController.getAll(Pickem3);

exports.getAllFilteredPicks = catchAsync(async (req, res, next) => {
  docs = await Pickem3.find();
...
exports.getPick = factoryController.getOne(Pickem3);
...
exports.getUsersPicks = catchAsync(async (req, res, next) => {
  ...
  const docs = await Pickem3.find({ participantsUsername: username });
...
exports.betPredictionResolver = catchAsync(async (req, res) => {
  const pickem3Url = req.body.url; // Get the URL from the request body
  ...
  const fetchPicks = async () => {
    try {
      return await axios.get(
        `${pickem3Url}/api/v1/pickem3/getPicksForPredicter`
      );
    } catch (error) {
      console.log(error);
    }
  ...
  for (const [index, pick] of picks.entries()) {
    ...
    if (
      ...
      try {
        ...
        const patchResponse = await Pickem3.findByIdAndUpdate(
          pick._id,
          updateData,
          { new: true }
        );
