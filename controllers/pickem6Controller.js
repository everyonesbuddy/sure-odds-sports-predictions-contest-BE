const Pickem6 = require('../models/pickem6Model');
const factoryController = require('./factoryController');
const catchAsync = require('../utils/catchAsync');
const AppError = require('../utils/appError');
const axios = require('axios');

exports.getAllPicks = factoryController.getAll(Pickem6);

exports.getAllFilteredPicks = catchAsync(async (req, res, next) => {
  docs = await Pickem6.find();
...
exports.getPick = factoryController.getOne(Pickem6);
...
exports.getUsersPicks = catchAsync(async (req, res, next) => {
  ...
  const docs = await Pickem6.find({ participantsUsername: username });
...
exports.betPredictionResolver = catchAsync(async (req, res) => {
  const pickem6Url = req.body.url; // Get the URL from the request body
  ...
  const fetchPicks = async () => {
    try {
      return await axios.get(
        `${pickem6Url}/api/v1/pickem6/getPicksForPredicter`
      );
    } catch (error) {
      console.log(error);
    }
  ...
  for (const [index, pick] of picks.entries()) {
    ...
    if (
      ...
      try {
        ...
        const patchResponse = await Pickem6.findByIdAndUpdate(
          pick._id,
          updateData,
          { new: true }
        );
