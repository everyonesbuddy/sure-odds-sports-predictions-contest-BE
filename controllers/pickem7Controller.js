const Pickem7 = require('../models/pickem7Model');
const factoryController = require('./factoryController');
const catchAsync = require('../utils/catchAsync');
const AppError = require('../utils/appError');
const axios = require('axios');

exports.getAllPicks = factoryController.getAll(Pickem7);

exports.getAllFilteredPicks = catchAsync(async (req, res, next) => {
  docs = await Pickem7.find();
...
exports.getPick = factoryController.getOne(Pickem7);
...
exports.getUsersPicks = catchAsync(async (req, res, next) => {
  ...
  const docs = await Pickem7.find({ participantsUsername: username });
...
exports.betPredictionResolver = catchAsync(async (req, res) => {
  const pickem7Url = req.body.url; // Get the URL from the request body
  ...
  const fetchPicks = async () => {
    try {
      return await axios.get(
        `${pickem7Url}/api/v1/pickem7/getPicksForPredicter`
      );
    } catch (error) {
      console.log(error);
    }
  ...
  for (const [index, pick] of picks.entries()) {
    ...
    if (
      ...
      try {
        ...
        const patchResponse = await Pickem7.findByIdAndUpdate(
          pick._id,
          updateData,
          { new: true }
        );
