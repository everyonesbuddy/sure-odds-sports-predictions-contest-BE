const Pickem5 = require('../models/pickem5Model');
const factoryController = require('./factoryController');
const catchAsync = require('../utils/catchAsync');
const AppError = require('../utils/appError');
const axios = require('axios');

exports.getAllPicks = factoryController.getAll(Pickem5);

exports.getAllFilteredPicks = catchAsync(async (req, res, next) => {
  docs = await Pickem5.find();
...
exports.getPick = factoryController.getOne(Pickem5);
...
exports.getUsersPicks = catchAsync(async (req, res, next) => {
  ...
  const docs = await Pickem5.find({ participantsUsername: username });
...
exports.betPredictionResolver = catchAsync(async (req, res) => {
  const pickem5Url = req.body.url; // Get the URL from the request body
  ...
  const fetchPicks = async () => {
    try {
      return await axios.get(
        `${pickem5Url}/api/v1/pickem5/getPicksForPredicter`
      );
    } catch (error) {
      console.log(error);
    }
  ...
  for (const [index, pick] of picks.entries()) {
    ...
    if (
      ...
      try {
        ...
        const patchResponse = await Pickem5.findByIdAndUpdate(
          pick._id,
          updateData,
          { new: true }
        );
